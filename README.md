...
•	В качестве валидатор-ноды (операторы узлов, управляющие бридж-нодами, имеют возможность также участвовать в консенсусе, став валидатором)
...
1.	Celestia App с Celestia Core (см. репозитарий)
...
•	Celestia Core – это уровень взаимодействия состояний, консенсуса и производства блоков. Celestia Core построено на основе Tendermint Core, модифицированного для хранения корней данных блоков со стирающим кодом, а также других изменений (см. ADRs).
2.	Celestia Node (см. репозиторий)
...
Этот гайд протестирован на примере сервера с ОС Ubuntu Linux 20.04 (LTS) x64.
...
Это установит важные пакеты, которые необходимы для выполнения многих задач, таких как загрузка файлов, компиляция и мониторинг ноды:
...
Развертывание приложения Celestia:
В этом разделе описана часть 1 настройки бридж-ноды Celestia: запуск демона Celestia App с внутренним узлом Celestia Core.

Note: Убедитесь, что у вас есть не менее 100+ Гб свободного места для безопасной установки и запуска бридж-ноды.
...
Для инициализации сети выберите "node-name", которое характеризует вашу ноду. Параметр --chain-id, который мы используем здесь, - devnet-2. Имейте в виду, что он может измениться, если будет развернута новая тестовая сеть.
...
Запустите приложение Celestia-App с помощью SystemD
...
Если все прошло успешно, вы должны увидеть результат, аналогичный следующему:
code: 0
codespace: ""
data: ""
gas_used: "0"
...
Предостережение: Для продолжения этого руководства вам понадобится запущенная celestia-app. Пожалуйста, обратитесь к celestia-app.md для установки.
...
Для того, чтобы ваша бридж-нода Celestia могла взаимодействовать с другими бридж-нодами, необходимо добавить их в качестве взаимных пиров в файл config.toml и разрешить обмен пирами. Пожалуйста, перейдите по адресу networks/devnet-2/celestia-node/mutual_peers.txt, чтобы найти список взаимных пиров.
Для получения более подробной информации о config.toml, пожалуйста, перейдите по этой ссылке.
...
tx: null
txhash: <tx-hash>
Теперь вы сможете найти своего валидатора в блок-эсклорере. Например, здесь.

Установка лайт-ноды Celestia
Это туториал по настройке лайт-ноды Celestia, которая сможет позволить вам выполнять сэмплирование данных в сети доступности данных (Data Availability, DA).
Обзор лайт-ноды
...
Примечание: В будущих имплементациях лайт-ноды смогут также публиковать транзакции (см. ADR), хотя в Devnet’e транзакциями занимаются бридж-ноды.

 

Лайт-ноды обладают следующими свойствами:
•	Подключение к бридж-нодам в сети доступности данных (DA). Примечание: лайт-ноды не общаются друг с другом, а только с бридж-нодами.
•	Получают ExtendedHeaders, т.е. обернутые "сырые" заголовки, которые уведомляют ноды Celestia о новых заголовках блоков и соответствующих метаданных DA.
•	Выполняют сэмплирование доступности данных (Data Availability Sampling, DAS) над полученными заголовками.
...
Более подробную информацию о config.toml можно найти здесь.
...
Откройте 2 терминала, чтобы посмотреть на то, как работает сэмплирование доступности данных:
•	В 1 терминале откройте журнал логов лайт-ноды
•	Во 2 термине используйте CLI приложения Celestia для отправки в сеть платной транзакции payForMessage
...
Ознакомьтесь с эксплорером здесь
...
Docker Compose
Одним из ресурсов для ознакомления с инфраструктурой Docker Compose для Celestia является репозиторий ephemeral-cluster. Этот репозиторий предоставляет инфраструктуру для проведения интеграционных тестов для разработки путем запуска экземпляра Docker Compose в Celestia devnet. Его не рекомендуется использовать как таковой для запуска нод по отдельности.
